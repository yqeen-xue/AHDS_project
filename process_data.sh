...
for file in ~/AHDS_project/data/raw/article-*.xml; do
    pmid=$(xmlstarlet sel -t -v "//PMID" "$file" | head -n 1)  # get PMID
    year=$(xmlstarlet sel -t -v "//PubDate/Year" "$file" | head -n 1)  # get year
    title=$(xmlstarlet sel -t -v "//ArticleTitle" "$file" | head -n 1)  # get title
